...
**Важно:** bootstrap пока не подключён к `app/table/page.tsx` — это следующий
интеграционный шаг. Текущий UI продолжает работать через прямые импорты правил.
...
| **rolls** | `modules/rolls/` | planned | types, system-adapter, configure, module-definition (UI ещё в table) |
| **character-sheet** | `modules/character-sheet/` | planned | types only |
| **journal** | `modules/journal/` | planned | types only |
| **reference** | `modules/reference/` | planned | types only |
...
| Область | Детали |
|---------|--------|
| `GameTable.tsx` | ~7.3k строк — оркестратор; hooks подключены, но VTM-логика ещё внутри |
| Прямые импорты VTM5 | `GameTable.tsx` и table components ещё импортируют `core/systems/vtm5/rules/*` напрямую |
| Hub bootstrap | `bootstrapChronicleRuntime` не вызывается из route — адаптеры не активны в runtime |
| Rolls module | lifecycle `planned`; UI бросков живёт в table |
| Infrastructure | `core/infrastructure/` — placeholder |
| Journal / Reference UI | компоненты в `components/journal`, `components/reference` |
| Character sheet | iframe bridge в `CharacterSheetScreen`, модуль — types only |
