...
| `ChronicleActor` | новая actor identity; optional link to existing `characters.id` | display profile может публиковаться; master tags/status private по policy | scoped master; published actor projection | не превращать NPC в `characters` автоматически; membership-aware RLS |
| `ActorPrivateFields` | 1:1/1:n с actor, отдельная master table | public отсутствует; secrets/motives/stats private | только master channel | отдельная таблица обязательна, чтобы player select не получил JSON secret |
